...
class OffergraphCommand(commands.Cog):
    ...
    async def offergraph(self, ctx: SlashContext, programme_id: str, year: int = None, approx: bool = True):
        if year is None:
        ...
        async with (await self.bot.get_db_conn()).acquire() as connection:
            ...
            try:
                await offers.generate_graph(programmes_helper.programmes[programme_id], not approx, year)
            except ValueError:
