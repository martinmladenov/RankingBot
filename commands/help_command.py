...
class HelpCommand(commands.Cog):
    ...
    async def help(self, ctx):
        await ctx.send(
            ...
            f'`.setofferdate <day> <month> <{programmes_helper.get_ids_string()}> [year]` Sets the date you\'ve received'
            f' an offer on Studielink to help other applicants predict when they might receive one\n'
            f'`.toggleprivaterank <{programmes_helper.get_ids_string()}> [year]` '
            f'Toggles whether your rank is displayed on `.ranks` for the specified programme\n'
